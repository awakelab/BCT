...
%   - Corrected display count number in trial
%
try
    ...
    if openwindow
        ...
        [win,rect] = Screen('OpenWindow',0,[0 0 0],[0 0 800 600]);
    end
    ...
    while GetSecs - starttime < duration
        [keypress keycode] = KbQueueCheck;
        if keypress % 1-8 press
            if keycode(KbName(inputkeys{1}))
            ...
            elseif keycode(KbName(inputkeys{2})) % 9 press
                ...
                %             end
                %             fprintf('%04.3f |   9   | %d\n',data(end,1),data(end,3));
            elseif keycode(KbName(inputkeys{3})) % reset press
    ...
    cutoffthreshold_singletrial = max(data(:,1));
